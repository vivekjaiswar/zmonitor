...
- **4 rows with real PASS evidence**: A1 (credential isolation, unit-level), B2 (transition/recovery decision logic, full truth-table), E1-E2 (single-monitor recovery + maintenance-suppression, same evidence as B2). All from this session's two new test files (`test-credential-isolation.js`, `test-monitor-transitions.js`), 16 assertions total, all passing.
