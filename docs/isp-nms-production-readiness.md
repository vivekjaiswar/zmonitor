...
| A2 | Credentials not exposed through Socket.IO | `getMonitorList`/`getMonitor`/`sendMonitorList` now gate on `socket.userRole === "admin"` | No automated integration test yet (would need a live server + two logged-in sockets, admin and employee) | Employee socket receives monitor JSON with no sensitive keys | **BLOCKED** | Code change made (`server/zmonitor-server.js`, `server/server.js`), logic traced by hand against every call site of `Monitor.toJSON` (3 total, confirmed via grep — no other caller exists). Not exercised against a running server in this session. |
