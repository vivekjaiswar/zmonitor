...
**Status:** **shipped** (commit `24a989c7`, branch `feature/isp-nms-production-foundation`). `server/model/monitor.js`, `server/zmonitor-server.js`, `server/server.js` — `includeSensitiveData` now defaults to `false` and is threaded from `socket.userRole === "admin"` at both call sites that build monitor payloads for clients. Regression test (`test/backend-test/test-credential-isolation.js`, 4/4 passing) proves the property directly. No live-server integration test yet — see `docs/isp-nms-production-readiness.md` row A2.
