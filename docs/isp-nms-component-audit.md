...
| Badges | `badge-maker` dep, `api-router.js` (6 distinct badge routes: status, uptime, ping, avg-response, cert-exp, response), `status-page-router.js` (overall status-page badge), `BadgeLinkGeneratorDialog.vue` | None — README shields.io-style badges | **DEPRECATE** (reclassified — was REMOVE Low-Medium) | **Medium**, corrected from Low-Medium after actually tracing it (2026-09-13) | Bigger footprint than first assessed: 6 routes across 2 files plus a dedicated dialog component, all built around a `badgeConstants` config object in `src/util.js`. This is also an externally-consumable URL scheme — an ISP customer's own README or a third-party status dashboard may already embed one of these badge URLs. Don't remove without confirming no external consumer depends on it; that confirmation can't come from grepping this repo. |
| Steam / gamedig (game server monitoring) | inline in `monitor.js`'s `beat()` (~line 782), `server/monitor-types/gamedig.js`, `gameList` picker in `EditMonitor.vue`, `general-socket-handler.js`'s `getGameList`, dedicated DB columns (`game`, `gamedig_token`) with their own Knex migrations (`2026-01-02-...gamedig-v4-to-v5.js`, `2026-05-25-...add-gamedig-token.js`) | None | **DEPRECATE** (reclassified — was REMOVE Low) | **Medium-High**, corrected from Low after actually tracing it (2026-09-13) | Removing this cleanly means editing the inline per-type branches in the same 2000-line `beat()` closure Phase 3 and Phase 4 both deliberately left untouched without an integration-test harness, dropping DB columns (needs a tested migration, per the project's own migration-safety rule — never just DROP COLUMN), and confirming no live ISP customer currently has a gamedig/Steam monitor configured (can't verify from this repo alone, would need to ask or check a real deployment). Not a "small, self-contained" removal once actually traced. |
...
|---|---|---|---|
| OS process name | `server/server.js:52` — `process.title = "uptime-kuma"` | **DONE** (2026-09-13, commit `4c46373b`) | Changed to `"zmonitor"` after tracing all 8 other "uptime-kuma" string hits in `server/` — confirmed all inert (GitHub issue/PR/advisory URL comments), none read `process.title` back. |
| Notification provider default values | `src/components/notifications/TechulusPush.vue` (`pushTitle`/`pushChannel` defaults) | **DONE** (2026-09-13, commit `4f003866`) | Cosmetic default-value fix, no migration needed. |
...
| Orphaned unused asset | `public/apple-touch-icon-precomposed.png` (Jun 4 timestamp, not referenced anywhere) | **DONE** (2026-09-13, commit `4c46373b`) | Re-confirmed zero references before deleting (grep across html/js/vue/json). |
...
- DEPRECATE / REMOVE (traced-but-not-yet-removed): 5 components remaining — status pages, badges, gaming monitors (Steam/gamedig), docker monitoring (pending investigation), website-content checks. Badges and gamedig/Steam were re-traced 2026-09-13 and reclassified from Low/Low-Medium risk to Medium/Medium-High — both are more entangled in `monitor.js`'s inline `beat()` logic and live-customer-data risk than the first pass assumed. Neither has a specific tested removal plan yet, so neither has been touched.
- Trivial branding cleanup (process.title, one notification-provider default, one orphaned asset): **done** (2026-09-13, commits `4c46373b`/`4f003866`) — these were genuinely zero-risk once traced, unlike badges/gamedig above.
...
- **Production-readiness gaps (§8-§12)**: self-health, stale telemetry, and flapping now have shipped classification/compute layers (see `docs/isp-nms-prd.md` §5.7-5.9) with unit-test evidence; global search and full backup/restore automation remain unbuilt. Tracking moved to `docs/isp-nms-production-readiness.md`, which is now the live status source — this document stays the audit/classification record.
- **Two original blocking findings (§0)**: no customer/subscriber schema exists yet (gates the wedge itself); the credential-exposure fix has shipped (commit `24a989c7`, see `docs/isp-nms-prd.md` §5.1) — no longer "in progress."
