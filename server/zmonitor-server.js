...
class ZMonitorServer {
    ...
    constructor() {
        ...
        ZMonitorServer.monitorTypeList["ntp"] = new NTPMonitorType();
    ...
    async sendMonitorList(socket) {
        let list = await this.getMonitorJSONList(socket.userID, null, socket.userRole === "admin");
    ...
    async sendUpdateMonitorIntoList(socket, monitorID) {
        let list = await this.getMonitorJSONList(socket.userID, monitorID, socket.userRole === "admin");
     ...
     * @param {boolean} includeSensitiveData - Include credentials/secrets
     * in each monitor's JSON. Only pass true for an admin caller - never
     * for a read-only/employee-scoped request.
     * @returns {Promise<object>} A promise that resolves to an object with monitor IDs as keys and monitor objects as values.
     *
     * Generated by Trelent
     */
    async getMonitorJSONList(userID, monitorID = null, includeSensitiveData = false) {
        const accessible = await Monitor.getAccessibleMonitorIdsSQL(userID);
        let query = ` id IN (${accessible.sql}) `;
        let queryParams = [...accessible.params];
        ...
        const result = {};
        monitorList.forEach((monitor) => (result[monitor.id] = monitor.toJSON(preloadData, includeSensitiveData)));
        return result;
    }
