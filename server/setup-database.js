 ...
 * @param {string} envName The base name of the environment variable (e.g., "ZMONITOR_DB_PASSWORD")
...
class SetupDatabase {
     ...
     * @param  {ZMonitorServer} server the main server instance
     */
    constructor(args, server) {
        ...
        if (process.env.ZMONITOR_DB_TYPE) {
            this.needSetup = false;
            log.info("setup-database", "ZMONITOR_DB_TYPE is provided by env, try to override db-config.json");
            dbConfig.type = process.env.ZMONITOR_DB_TYPE;
            dbConfig.hostname = process.env.ZMONITOR_DB_HOSTNAME;
            dbConfig.port = process.env.ZMONITOR_DB_PORT;
            dbConfig.dbName = process.env.ZMONITOR_DB_NAME;
            dbConfig.username = getEnvOrFile("ZMONITOR_DB_USERNAME");
            dbConfig.password = getEnvOrFile("ZMONITOR_DB_PASSWORD");
            dbConfig.socketPath = process.env.ZMONITOR_DB_SOCKET?.trim();
            dbConfig.ssl = getEnvOrFile("ZMONITOR_DB_SSL")?.toLowerCase() === "true";
            dbConfig.ca = getEnvOrFile("ZMONITOR_DB_CA");
    ...
    isEnabledEmbeddedMariaDB() {
        return process.env.ZMONITOR_ENABLE_EMBEDDED_MARIADB === "1";
    }
        ...
        return new Promise((resolve) => {
            ...
            app.get("/setup-database-info", (request, response) => {
                ...
                response.json({
                    ...
                    isEnabledMariaDBSocket: process.env.ZMONITOR_DB_SOCKET?.trim().length > 0,
                });
            });
            ...
            app.post("/setup-database", async (request, response) => {
                ...
                if (dbConfig.type === "mariadb") {
                    ...
                    if (process.env.ZMONITOR_DB_SOCKET?.trim().length > 0) {
                        dbConfig.socketPath = process.env.ZMONITOR_DB_SOCKET.trim();
                    } else {
