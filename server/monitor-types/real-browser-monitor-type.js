...
async function isAllowedChromeExecutable(executablePath) {
    if (config.args["allow-all-chrome-exec"] || process.env.ZMONITOR_ALLOW_ALL_CHROME_EXEC === "1") {
        return true;
    }
...
async function prepareChromeExecutable(executablePath) {
    ...
    if (typeof executablePath === "string" && executablePath.toLocaleLowerCase() === "#playwright_chromium") {
        ...
        if (!(await isAllowedChromeExecutable(executablePath))) {
            throw new Error(
                "This Chromium executable path is not allowed by default. If you are sure this is safe, please add an environment variable ZMONITOR_ALLOW_ALL_CHROME_EXEC=1 to allow it."
            );
        }
    }
